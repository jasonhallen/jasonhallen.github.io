...
function draw() {
  ...
  if (sample_list.length !== samples_list.length) {
...
// function drawWaveform(sample) {
  //Generate waveform graphic
  //Append the graphic to the sample_list entry
// }
...
function ControlPanel(track) {
  ...
  this.sample_change = () => {
    ...
    this.track.wavePanel.generateWaveform();
  }
...
function WavePanel(track) {
  ...
  this.generateWaveform = function() {
    if (this.track.sample.wave_form === undefined) {
      this.track.sample.wave_form = createGraphics(this.width, this.height);
      this.track.sample.wave_form.strokeWeight(3);
      this.track.sample.wave_form.stroke(255);
      for (var i = 0; i < this.track.sample.peaks.length; i = i + 6) {
        var x = map(i, 0, this.track.sample.peaks.length, 0, this.width);
        var max = map(this.track.sample.peaks[i], 1, -1, 0, this.height);
        var min = map(this.track.sample.peaks[i + 1], 1, -1, 0, this.height);
        this.track.sample.wave_form.strokeCap(SQUARE);
        this.track.sample.wave_form.line(x, this.height / 2, x, max);
        this.track.sample.wave_form.line(x, this.height / 2, x, min);
      }
    }
  }
  ...
  this.display = function() {
    ...
    // }
    image(this.track.sample.wave_form, this.x_position, this.y_position);
...
function Mixer() {
  ...
  this.mixer_graphic = createGraphics(this.width, this.height);
  this.mixer_graphic.textFont(roboto);
  this.mixer_graphic.textStyle(NORMAL);
  // this.mixer_graphic.position(this.x_position, this.y_position, "fixed");

  this.display = function() {
    // this.mixer_graphic.fill(255);
    // this.mixer_graphic.textSize(24);
    // this.mixer_graphic.text("MIXER",100,100);
    // image(this.mixer_graphic, this.x_position, this.y_position);
    ...
    rect(this.x_position + 70, this.y_position + 260, 26, map(this.right_meter, 0, 1.5, 0, -210));
